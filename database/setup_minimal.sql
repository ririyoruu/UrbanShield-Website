-- Minimal Setup for Incident Moderation
-- Run this after checking what you have with the diagnostic script

-- 1. Add missing columns to incidents table (if they don't exist)
DO $$
BEGIN
  -- Add status column
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added status column to incidents table';
  END IF;
  ...
  -- Add assigned_responder_id column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
