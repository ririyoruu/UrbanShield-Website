-- Setup Script for Database with ENUM Status
-- Handles existing incident_status ENUM properly

-- 1. First, let's see what ENUM values exist
SELECT 
  'ENUM_VALUES' as section,
...
-- 2. Add new ENUM values if they don't exist
DO $$
BEGIN
  -- Check if incident_status enum exists and add missing values
  IF EXISTS (
    SELECT 1 FROM pg_type WHERE typname = 'incident_status'
  ) THEN
    -- Add new enum values if they don't exist
    PERFORM add_enum_value('incident_status', 'pending') 
    WHERE NOT EXISTS (
    ...
    PERFORM add_enum_value('incident_status', 'in_action') 
    WHERE NOT EXISTS (
      SELECT 1 FROM pg_enum 
      WHERE enumtypid = (SELECT oid FROM pg_type WHERE typname = 'incident_status')
      AND enumlabel = 'in_action'
    );
    
    PERFORM add_enum_value('incident_status', 'resolved') 
    WHERE NOT EXISTS (
      SELECT 1 FROM pg_enum 
      WHERE enumtypid = (SELECT oid FROM pg_type WHERE typname = 'incident_status')
    ...
    PERFORM add_enum_value('incident_status', 'duplicate') 
    WHERE NOT EXISTS (
      SELECT 1 FROM pg_enum 
      WHERE enumtypid = (SELECT oid FROM pg_type WHERE typname = 'incident_status')
...
-- 3. Now update existing 'open' status to 'pending'
UPDATE incidents 
SET status = 'pending' 
WHERE status = 'open';

-- 4. Update existing records based on is_verified field
UPDATE incidents 
SET status = CASE 
...
-- 5. Add missing columns to incidents table (only if they don't exist)
DO $$
BEGIN
...
-- 9. Create RLS policies for new tables (based on your user_type field)
...
-- 10. Create functions for the moderation workflow
...
-- 13. Update existing resolved incidents
UPDATE incidents 
SET 
...
-- 14. Confirmation query - shows what was updated
SELECT 
